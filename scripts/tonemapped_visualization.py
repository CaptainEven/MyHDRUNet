...
for file_name in sorted(os.listdir(ref_dir)):
    ...
    hdr_image = io.imread_uint16_png(osp.join(ref_dir,
                                              "{:04d}_gt.png".format(image_id)),
                                     osp.join(ref_alignratio_dir, "{:04d}_alignratio.npy".format(image_id)))
    ...
    res_image = io.imread_uint16_png(osp.join(res_dir, "{:04d}.png".format(image_id)),
                                     osp.join(res_alignratio_dir, "{:04d}_alignratio.npy".format(image_id)))
    res_linear_image = res_image ** 2.24
    res_image = (m.tanh_norm_mu_tonemap(res_linear_image, norm_perc) * 255.).round().astype(np.uint8)
    cv2.imwrite(osp.join(res_output_dir, "{:04d}_tone_mapped_result.png".format(image_id)),
                cv2.cvtColor(res_image, cv2.COLOR_RGB2BGR))
